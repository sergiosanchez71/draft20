...
- **Publicidad (AdSense)**: el script de Google AdSense (`ADSENSE_CLIENT` en `inc/layout.php`) va en el `<head>` de todas las páginas de contenido (no en `juego.php`, que es vista de app con `noindex`). La CSP (`csp_policy()`) permite los dominios de AdSense y de la CMP de Google (`fundingchoicesmessages.google.com`); `ads.txt` declara el publisher y la privacidad (`lang/es.json` → `priv_*`) informa de las cookies de terceros y de cómo retirar el consentimiento (CMP en AdSense → Privacidad y mensajería).
