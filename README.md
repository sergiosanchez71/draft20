...
- **Partida rápida (matchmaking)**: `api/partida_rapida.php` empareja en el lobby con el primero que también pulse "Partida rápida" (temática aleatoria, sin compartir código). Cola en `api/datos/cola_rapida.json` con purga de entradas muertas (sala inexistente/empezada u oyente sin poll > 60 s); si nadie entra, se ofrece practicar contra el bot (con dificultad) sin dejar de buscar: la sala y el enlace siguen vivos y, si alguien se une mientras practicas, la partida del bot avisa con cuenta atrás ("{nombre} se ha unido · entras en 5 s", con botón **Entrar ya**) y te lleva a la partida humana abandonando la de práctica.
